...
| **P1 — en progreso** (acotado) | Step de **aprobación humana** dentro de un workflow (esperar que una persona apruebe antes de continuar) | SuiteCRM (AOW) | El resto de "automatización robusta" ya existe (`modules/workflow`: condicionales, loops, formularios, HTTP, cron, webhooks, paso de IA). El step `FORM` ya tiene el mecanismo genérico de pausa/reanudación que se reutiliza — diseño y avance real en [`docs/WORKFLOW_APPROVAL_DESIGN.md`](./WORKFLOW_APPROVAL_DESIGN.md) |
...
| **P2 — Fase 1 lista** (acotado) | Click-to-call e integración de proveedores VoIP | EspoCRM | Confirmado: no había ninguna integración de telefonía en el repo (los bots `call-recorder`/`fireflies` graban videollamadas agendadas, no marcan teléfonos). Fase 1 (link `tel:`, sin proveedor) implementada como Application — ver [`docs/CLICK_TO_CALL_DESIGN.md`](./CLICK_TO_CALL_DESIGN.md). Fase 2 (softphone embebido con un proveedor real) requiere una decisión de negocio pendiente (qué proveedor, si alguno) |
